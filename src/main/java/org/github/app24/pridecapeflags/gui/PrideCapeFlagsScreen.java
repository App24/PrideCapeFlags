...
import org.github.app24.pridecapeflags.*;
...
public class PrideCapeFlagsScreen extends Screen {
    ...
    private static final Identifier BACKGROUND = Identifier.fromNamespaceAndPath(PrideCapeFlagsMod.MODID, "textures/gui/cape_selection_bg.png");
    ...
    protected void init() {
...

        {
            var resetButton = Button.builder(Component.literal("R"), button -> {
                        this.capeFlagBox.setValue(Config.PRIDE_CAPE_FLAG.get());
                    })
                    .size(20, 20).build();
            ...
            this.capeFlagBox.setResponder(s -> {
                resetButton.active = !this.capeFlagBox.getValue().equals(Config.PRIDE_CAPE_FLAG.get());
                updateApplyButton();
            });
        ...
        {
            var resetButton = Button.builder(Component.literal("R"), button -> {
                        this.elytraFlagBox.setValue(Config.ELYTRA_PRIDE_CAPE_FLAG.get());
                    })
                    .size(20, 20).build();
            ...
            this.elytraFlagBox.setResponder(s -> {
                resetButton.active = !this.elytraFlagBox.getValue().equals(Config.ELYTRA_PRIDE_CAPE_FLAG.get());
                updateApplyButton();
            });
        ...
        this.capeFlagBox.setValue(Config.PRIDE_CAPE_FLAG.get());
        this.showCape = Config.SHOW_PRIDE_CAPE.get();
        this.elytraFlagBox.setValue(Config.ELYTRA_PRIDE_CAPE_FLAG.get());

        this.useElytraCape = Config.USE_ELYTRA_CAPE.get();
...

        {
            ...
            useElytraButton = CycleButton.onOffBuilder(useElytraCape).create(capeFlagBoxX, yP + 30, 150, 20, Component.translatable("pridecapeflags.configuration.use_elytra_cape"), (button, value) -> {
                this.useElytraCape = value;
                resetButton.active = this.useElytraCape != Config.USE_ELYTRA_CAPE.get();
                this.elytraFlagBox.setEditable(this.useElytraCape);
                updateApplyButton();
            });
...

        {
            ...
            showCapeButton = CycleButton.onOffBuilder(this.showCape).create(capeFlagBoxX, yP - 40, 150, 20, Component.translatable("pridecapeflags.config.menu.showCape"), (button, value) -> {
                this.showCape = value;
                resetButton.active = this.showCape != Config.SHOW_PRIDE_CAPE.get();
                updateApplyButton();
            });
    ...
    private void saveSettings() {
        if (hasSettingsChanged()) {
            Config.PRIDE_CAPE_FLAG.set(this.capeFlagBox.getValue());
            Config.ELYTRA_PRIDE_CAPE_FLAG.set(this.elytraFlagBox.getValue());
            Config.USE_ELYTRA_CAPE.set(this.useElytraCape);
            Config.SHOW_PRIDE_CAPE.set(this.showCape);
            Config.SPEC.save();
        }
    ...
    private boolean hasSettingsChanged() {
        return this.useElytraCape != Config.USE_ELYTRA_CAPE.get() ||
                this.showCape != Config.SHOW_PRIDE_CAPE.get() ||
                !this.capeFlagBox.getValue().equals(Config.PRIDE_CAPE_FLAG.get()) ||
                !this.elytraFlagBox.getValue().equals(Config.ELYTRA_PRIDE_CAPE_FLAG.get());
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float a) {
        ...
        this.capeFlagBox.render(graphics, mouseX, mouseY, a);
        this.elytraFlagBox.render(graphics, mouseX, mouseY, a);
    }
    ...
    private void extractEntityInInventoryFollowsMouse(final GuiGraphics graphics, final int x0, final int y0, final int x1, final int y1, final int size, float offsetY) {
        ...
        if (this.showCape) {
            ...
            if (capeResourceLocation != null) {
                if (!PrideCapeFlagsModClient.checkFlagValid(capeResourceLocation))
